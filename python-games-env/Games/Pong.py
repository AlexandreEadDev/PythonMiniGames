...
BALL_RADIUS = 10
SCORE_FONT = pygame.font.SysFont("comicsans", 38)
FINAL_SCORE = 20
...
def draw(windows, paddles, ball, left_score, right_score):
    windows.fill(BLACK)

    left_score_text = SCORE_FONT.render(f"{left_score}", 1, WHITE)
    right_score_text = SCORE_FONT.render(f"{right_score}", 1, WHITE)
    windows.blit(left_score_text, (WIDTH//4 -
                 left_score_text.get_width()//2, 20))
    windows.blit(right_score_text, (WIDTH*3 // 4 -
...
def main():
    ...
    while run:
        ...
        elif right_score >= FINAL_SCORE:
            won = True
            win_text = "Player 2 Win"

        if won:
            text = SCORE_FONT.render(win_text, 1, RED)
            WINDOWS.blit(text, (WIDTH//2 - text.get_width() //
